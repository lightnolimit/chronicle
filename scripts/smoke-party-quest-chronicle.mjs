#!/usr/bin/env node
/**
 * Chronicle Development Party Quest smoke test (shared Phantasy agents).
 *
 * Env:
 *   PARTY_QUEST_URL
 *   AGENT — phantasy-code | phantasy-debug | phantasy-marketing | phantasy-research
 *   CAMPAIGN_SLUG — default chronicle-development
 *   CHRONICLE_REPO — path to repo (default ~/chronicle-ops/chronicle)
 *   CREDENTIALS_ENV — phantasy agent .env with PARTY_QUEST_API_KEY
 *
...
async function main() {
  ...
  const agentId = process.env.AGENT?.trim() || "phantasy-code";
  ...
  const apiKey = process.env.PARTY_QUEST_API_KEY || credentialsEnv.PARTY_QUEST_API_KEY;
  if (!apiKey) {
    throw new Error("PARTY_QUEST_API_KEY required (env or CREDENTIALS_ENV)");
  }

  const questTitle = process.env.QUEST_TITLE?.trim() || profile.defaultQuest;
  const campaignSlug = process.env.CAMPAIGN_SLUG?.trim() || "chronicle-development";
  const repoPath = resolve(
    process.env.CHRONICLE_REPO?.trim() || join(homedir(), "chronicle-ops/chronicle"),
  );
  ...
  if (!assignment?.runId || !assignment?.quest) {
    throw new Error(
      `No assignment claimed. Heartbeat: ${JSON.stringify(heartbeat, null, 2)}`,
    );
  }
  ...
  const evidence = {
    schemaVersion: "chronicle.party-quest.development-smoke.v1",
  ...
  await postJson(
    joinUrl(partyQuestUrl, WEBHOOK_PATHS.runTrace),
    {
      ...
      events: [
        {
          eventType: "chronicle.development.exec",
          status: "info",
...
async function reportSuccess(partyQuestUrl, apiKey, assignment, { summary, message }) {
  await postJson(
    joinUrl(partyQuestUrl, WEBHOOK_PATHS.runTrace),
    {
      ...
      events: [
        {
          eventType: "chronicle.development.claim",
          status: "info",
          message,
          timestamp: Date.now(),
        },
...
function writeEvidence(evidence) {
  ...
  const appendPath =
    ...
    join(outDir, `chronicle-development-smoke-${day}.jsonl`);
  appendFileSync(appendPath, `${JSON.stringify(evidence)}\n`);
  writeFileSync(
    join(outDir, `chronicle-development-smoke-${day}.json`),
